    ...
    <script>
        ...
        function redirectToCreateView() {
            ...
            if (debutReservation && finReservation) {
                const reservationId = '{{ $reservation->id }}'; // Inject the reservation ID from the backend
                window.location.href = `{{ url('reservations') }}/${reservationId}/edit?dateDepart=${debutReservation}&dateRetour=${finReservation}`;
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Trigger price calculation on page load (in case there's already a selected car)

            const form = document.querySelector('form');
            form.addEventListener('submit', function(event) {
                updateHiddenAccessoryInput(); // Ensure this updates before submitting
                console.log('Accessoires Data Before Submit:', document.getElementById('accessoiresData').value); // Log it to check
            });
            ...
            <form action="{{ route('reservations.update', $reservation->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="debutReservation" id="debutReservation" 
                        value="{{ old('debutReservation', $reservation->debutReservation ? $reservation->debutReservation->format('Y-m-d') : '') }}" 
                        required>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="finReservation" id="finReservation" 
                        value="{{ old('finReservation', $reservation->finReservation ? $reservation->finReservation->format('Y-m-d') : '') }}" 
                        required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="pickUp" id="pickUp" value="{{ old('pickUp', $reservation->pickUp) }}" required>
                    @error('pickUp')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="dropOff" id="dropOff" value="{{ old('dropOff', $reservation->dropOff) }}" required>
                    @error('dropOff')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="statut" id="statut" required>
                        ...
                        <option value="confirmed" {{ old('statut', $reservation->statut) == 'confirmed' ? 'selected' : '' }}>confirmed</option>
                        <option value="pending" {{ old('statut', $reservation->statut) == 'pending' ? 'selected' : '' }}>pending</option>
                        <option value="cancelled" {{ old('statut', $reservation->statut) == 'cancelled' ? 'selected' : '' }}>cancelled</option>
                    </select>
                    @error('statut')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <select name="client_id" id="client" required>
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('client_id', $reservation->client_id) == $client->id ? 'selected' : '' }}>{{ $client->user->name }}</option>
                        @endforeach
                    </select>
                    @error('client_id')
                        <div class="error-message">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <select name="secondDriver" id="secondDriver">
                        <option value="" disabled>Choisir un client</option>
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('secondDriver', $reservation->secondDriver) == $client->id ? 'selected' : '' }}>{{ $client->user->name }}</option>
                        @endforeach
                    </select>
                    @error('secondDriver')
                        <div class="error-message">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="codeContrat" id="codeContrat" value="{{ old('codeContrat', $reservation->codeContrat) }}" required>
                    @error('codeContrat')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="voiture_id" id="voiture" required>
                        ...
                        @foreach($voitures as $voiture)
                            <option value="{{ $voiture->id }}" data-price="{{ $voiture->prices->first()->prix }}" 
                                {{ old('voiture_id', $reservation->voiture_id) == $voiture->id ? 'selected' : '' }}>
                                {{ $voiture->marque }} - {{ $voiture->modele }} - {{ $voiture->prices->first()->prix }} MAD
                            </option>
                        @endforeach
                    </select>
                        ...
                        @foreach($accessoires as $accessoire)
                            <div class="accessoire-card">
                                <div class="accessoire-header">
                                    <img src="{{ Storage::url($accessoire->image) }}" alt="{{ $accessoire->nom }}" class="accessoire-image">
                                    ...
                                    <div class="accessoire-quantity">
                                        ...
                                        <select name="accessoires[{{ $accessoire->id }}][quantity]" id="quantity_{{ $accessoire->id }}">
                                            @for ($i = 0; $i <= $accessoire->max; $i++)
                                                <option value="{{ $i }}"
                                                    {{ isset($reservation->accessoires) && $reservation->accessoires->contains($accessoire->id) && $reservation->accessoires->find($accessoire->id)->pivot->quantite == $i ? 'selected' : '' }}>
                                                    {{ $i }}
                                                </option>
                                            @endfor
                                        </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="total" id="total" value="{{ old('total', $reservation->total) }}" required readonly>
                    @error('total')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
